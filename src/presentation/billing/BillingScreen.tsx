...
import { Search, Plus, Minus, Trash2, ShoppingCart, X, CreditCard, Banknote, QrCode, Printer } from 'lucide-react';
...
export function BillingScreen() {
  ...
  const handleCheckout = async (printReceipt: boolean = false) => {
    ...
    try {
      ...
      const billNo = `INV-${dateStr}-${(count + 1).toString().padStart(4, '0')}`;
      ...
      if (printReceipt) {
        try {
          ...
          const receiptData = buildReceipt(billData, billItemsData, shopName, paperSize);
  ...
  return (
    <div className="flex h-full md:h-screen overflow-hidden bg-[#F2F2F7]">
      ...
      <div className={cn(
        "fixed md:static inset-y-0 right-0 z-50 w-full md:w-96 bg-white shadow-[-4px_0_24px_rgba(0,0,0,0.05)] flex flex-col transition-transform duration-300 ease-in-out",
        isCartOpen ? "translate-x-0" : "translate-x-full md:translate-x-0"
      )}>
        {/* Cart Header */}
        <div className="p-4 border-b border-[#C6C6C8]/50 flex items-center justify-between bg-white">
          <h2 className="text-xl font-bold text-[#1C1C1E]">Current Bill</h2>
        ...
        {/* Customer Info */}
        <div className="p-4 border-b border-[#C6C6C8]/50 flex gap-2">
          <input 
            type="text" 
        ...
        {/* Totals & Checkout */}
        <div className="bg-white border-t border-[#C6C6C8]/50 p-4 pb-safe space-y-4 shadow-[0_-4px_24px_rgba(0,0,0,0.05)]">
